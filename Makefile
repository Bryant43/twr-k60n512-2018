...
# toolchain specific
TOOLCHAIN = /usr/local/gcc-arm-none-eabi-7-2018-q2-update/bin/arm-none-eabi-
CC = $(TOOLCHAIN)gcc
CXX = $(TOOLCHAIN)g++
...
GDBSERVER = /usr/local/bin/openocdosbdm -d 0 -c "interface osbdm" -f /usr/local/share/openocd/scripts/board/twr-k60n512.cfg
...
print_size:
	@echo 'Printing size'
	@echo '-------------'
	$(SIZE) --totals $(OBJ_FOLDER)$(TARGET).$(TARGET_EXT); $(OBJCP) -O binary $(OBJ_FOLDER)$(TARGET).$(TARGET_EXT) $(OBJ_FOLDER)$(TARGET).bin ;
	@echo ' '
